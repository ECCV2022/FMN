...
class CUB200(Dataset):
    ...
    def __init__(self, root='./', train=True,
                 index_path=None, index=None, base_sess=None, shot=None):
        ...
        if train:
            ...
            if base_sess:
                ...
                self.data, self.targets = self.SelectfromTxt(self.data2label, index_path)
                # self.data, self.targets = self.ShotSelectfromClasses(self.data, self.targets, index, shot)
        else:
            self.transform = transforms.Compose([
                transforms.Resize(256),
                transforms.CenterCrop(224),
    ...
    def ShotSelectfromClasses(self, data, targets, index, shot):
        data_tmp = []
        targets_tmp = []
        for i in index:
            ind_cl = np.where(i == targets)[0][:shot]
            for j in ind_cl:
                data_tmp.append(data[j])
                targets_tmp.append(targets[j])
        print('in shot select, data_tmp size is :', len(data_tmp))
        return data_tmp, targets_tmp
