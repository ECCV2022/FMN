...
def get_base_dataloader(args):
    ...
    class_index = np.arange(args.base_class)#np.arange(100)######
    if args.dataset == 'cifar100':
    ...
    #     print('Kmeans loading...',trainset.__len__())
...
def get_new_dataloader(args,session):
    ...
    class_index = np.arange(args.base_class)
    class_index_current = get_session_classes_current(args, session)
    if args.dataset == 'cifar100':
        class_index = open(txt_path).read().splitlines()
        trainset = args.Dataset.CIFAR100(root=args.dataroot, train=True, download=False,
                                         index_path=txt_path, index=class_index_current, base_sess=False)
    if args.dataset == 'cub200':
        # print(class_index_current)
        trainset = args.Dataset.CUB200(root=args.dataroot, train=True,
                                       index_path=txt_path, index=class_index_current, shot=args.shot)
    if args.dataset == 'mini_imagenet':
        trainset = args.Dataset.MiniImageNet(root=args.dataroot, train=True,
                                       index_path=txt_path)
